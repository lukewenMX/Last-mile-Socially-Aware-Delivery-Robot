...
StateLatticePlanner::SamplingParams::SamplingParams(const int _n_p,
                                                    const int _n_h,
                                                    const double _length,
                                                    const double _max_alpha,
                                                    const double _max_psi) {
  n_p = _n_p;
  n_h = _n_h;
  length = _length;
...
void StateLatticePlanner::generate_biased_polar_states(const int n_s,
...
bool StateLatticePlanner::generate_trajectories(const std::vector<Eigen::Vector3d> &boundary_states,
...
#pragma omp parallel for
  for (int i = 0; i < trajectory_num; i++) {
    ...
    MotionModelDiffDrive::ControlParams init(MotionModelDiffDrive::VelocityParams(velocity,
                                                                                  MAX_ACCELERATION,
                                                                                  target_velocity,
                                                                                  target_velocity,
                                                                                  MAX_ACCELERATION),
                                             MotionModelDiffDrive::AngularVelocityParams(k0,
                                                                                         param.omega.km,
                                                                                         param.omega.kf,
                                                                                         param.omega.sf));
    ...
    double cost = tg.generate_optimized_trajectory(boundary_state,
                                                   init,
                                                   1.0 / HZ,
...
bool StateLatticePlanner::pickup_trajectory(const std::vector<MotionModelDiffDrive::Trajectory> &candidate_trajectories,
                                            const Eigen::Vector3d &goal,
                                            const MotionModelDiffDrive::Trajectory &last_trajectory,
                                            MotionModelDiffDrive::Trajectory &output, double social) {
  /*
  ...
  auto compare_trajectories =
      [=](const MotionModelDiffDrive::Trajectory &lhs, const MotionModelDiffDrive::Trajectory &rhs) {
  ...
  for (int i = 0; i < N; i++){
    ...
    for(int j = 0;j < trajectories[i].trajectory.size();++j){
      average_y += trajectories[i].trajectory[j](1);
    }
    average_y /= trajectories[i].trajectory.size();
    std::cout << "average_y: " << average_y * 100<< std::endl;
    social_costs.push_back(average_y);
  }
  for (int i = 0; i < N; i++) {
    double diff_traj = calculate_diff_trajectory(trajectories[i], last_trajectory);
    double diff_yaw = trajectories[i].trajectory.back()(2) - goal(2);
    double diff_goal = (trajectories[i].trajectory.back().segment(0,2) - goal.segment(0,2)).norm();
    double cost = fabs(atan2(sin(diff_yaw), cos(diff_yaw))) + 5 * diff_traj + diff_goal - social_costs[i] * social;
    // double cost = fabs(atan2(sin(diff_yaw), cos(diff_yaw))) + diff_traj + diff_goal + trajectories[i].cost;
    printf("cost: %lf, diff traj: %lf, diff goal: %lf, diff yaw: %lf\n", cost, diff_traj, diff_yaw, diff_goal);
    if (min_cost > cost) {
      min_cost = cost;
...
double StateLatticePlanner::calculate_diff_trajectory(const MotionModelDiffDrive::Trajectory &cur,
                                                      const MotionModelDiffDrive::Trajectory &last) {
  if (last.trajectory.empty()) {
    return 0.0;
  }
  double diff = 0;
  int mini_N = std::min(last.trajectory.size(), cur.trajectory.size());

  for (int i = 0; i < mini_N; ++i) {
    diff += std::hypot(last.trajectory[i](0) - cur.trajectory[i](0), last.trajectory[i](1) - cur.trajectory[i](1));
  }
  diff = diff / (double)mini_N;
  return diff;
}


void StateLatticePlanner::generate_bresemhams_line(const std::vector<Eigen::Vector3d> &trajectory,
...
bool StateLatticePlanner::check_collision(const state_lattice_planner::ObstacleMap<int> &map,
                                          MotionModelDiffDrive::Trajectory& trajectory) {
  /*
  ...
  generate_bresemhams_line(trajectory.trajectory, resolution, bresenhams_line);
  ...
  for (int i = 0; i < size; i++) {
   ...
   if (map.data[index] == 101) {
       trajectory.cost = 200;
   } else {
       trajectory.cost = 0;
   }
    if (map.data[index] != 0 && map.data[index] != 101) {
      return true;
    }
  }
  return false;
...
bool StateLatticePlanner::check_collision(const state_lattice_planner::ObstacleMap<int> &map,
  ...
  for (int i = 0; i < size; i++) {
    ...
    if (map.data[index] != 0) {
      std::cout << "index x: " << bresenhams_line[i](0) << ", index y: " << bresenhams_line[i](1) << std::endl;
