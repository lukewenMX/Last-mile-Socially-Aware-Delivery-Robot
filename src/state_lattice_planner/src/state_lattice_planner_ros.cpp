...
StateLatticePlannerROS::StateLatticePlannerROS(void)
  ...
  local_nh.param("SOCIAL_COST",SOCIAL_COST,{0});
  local_nh.param("LOOKUP_TABLE_FILE_NAME",
                 LOOKUP_TABLE_FILE_NAME,
                 {std::string(std::getenv("HOME")) + "/lookup_table.csv"});
  ...
  candidate_trajectories_no_collision_pub =
      local_nh.advertise<visualization_msgs::MarkerArray>("candidate_trajectories/no_collision", 1);
  selected_trajectory_pub = local_nh.advertise<visualization_msgs::Marker>("selected_trajectory", 1);
  occupancy_grid_pub = local_nh.advertise<nav_msgs::OccupancyGrid>("planning_occupancy_grid", 1);

  pre_sub = nh.subscribe("/local_prediction_trajectory", 1, &StateLatticePlannerROS::pre_callback, this);
  joy_sub = nh.subscribe("/joy",1,&StateLatticePlannerROS::joy_callback, this);
  local_goal_sub = nh.subscribe("/move_base_simple/goal", 1, &StateLatticePlannerROS::local_goal_callback, this);
  local_map_sub = nh.subscribe("/occmap", 1, &StateLatticePlannerROS::local_map_callback, this);
  odom_sub = nh.subscribe("/robot_1/odom", 1, &StateLatticePlannerROS::odom_callback, this);
...
void StateLatticePlannerROS::pre_callback(const sidewalk_msgs::Trajectories &msg) {
    pedestrian_prediction_ = msg;
}
...
void StateLatticePlannerROS::joy_callback(const sensor_msgs::JoyConstPtr& msg){
  joy_enabled = msg->buttons[0] == 1;
}
...
void StateLatticePlannerROS::process(void) {
  ...
  while (ros::ok()) {
    ...
    if (local_goal_subscribed && local_map_updated && odom_updated && goal_transformed) {
      ...
      Eigen::Vector3d goal(local_goal_base_link.pose.position.x,
                           local_goal_base_link.pose.position.y,
      ...
      bool generated = planner.generate_trajectories(states,
                                                     current_velocity.linear.x,
                                                     current_velocity.angular.z,
                                                     target_velocity,
                                                     trajectories);
      if (ENABLE_SHARP_TRAJECTORY) {
        generated |= planner.generate_trajectories(states,
                                                   current_velocity.linear.x,
                                                   current_velocity.angular.z + MAX_D_YAWRATE / HZ,
                                                   target_velocity,
                                                   trajectories);
        generated |= planner.generate_trajectories(states,
                                                   current_velocity.linear.x,
      ...
      if (generated) {
        ...
        get_obstacle_map(local_map, pedestrian_prediction_, obstacle_map);
        for (auto &trajectory: trajectories) {
          if (!planner.check_collision(obstacle_map, trajectory)) {
        ...
        if (candidate_trajectories.size() > 0) {
          visualize_trajectories(candidate_trajectories,
                                 0,
                                 0.5,
                                 1,
                                 last_trajectory_num,
          ...
          planner.pickup_trajectory(candidate_trajectories, goal, last_trajectory_, trajectory, SOCIAL_COST);
          ...
          int delayed_control_index =
              std::min(std::ceil(calculation_time * HZ) + CONTROL_DELAY, (double) trajectory.trajectory.size());
          ...
          cmd_vel.linear.x = trajectory.velocities[delayed_control_index] * 0.4;
          cmd_vel.angular.z = trajectory.angular_velocities[delayed_control_index] * 0.7;
          if(joy_enabled){
          ...
          if(joy_enabled){
            ...
            std::cout << "joy not enabled!" << std::endl;
          }
          // for clear
          ...
          visualize_trajectories(clear_trajectories,
                                 0,
                                 0.5,
                                 1,
                                 last_trajectory_num,
...
void StateLatticePlannerROS::visualize_trajectories(const std::vector<MotionModelDiffDrive::Trajectory> &trajectories,
                                                    const double r,
                                                    const double g,
                                                    const double b,
...
void StateLatticePlannerROS::visualize_trajectory(const MotionModelDiffDrive::Trajectory &trajectory,
                                                  const double r,
                                                  const double g,
                                                  const double b,
                                                  const ros::Publisher &pub) {
...
void StateLatticePlannerROS::get_obstacle_map(const nav_msgs::OccupancyGrid &input_map,
                                              const sidewalk_msgs::Trajectories& ped_pre,
                                              state_lattice_planner::ObstacleMap<TYPE> &output_map) {
  ...
  double resolution = output_map.get_resolution();
  for (const auto& iter : ped_pre.trajectories) {
    std::vector<Eigen::Vector3d> trajectory;
    for (const auto& it : iter.position) {
      Eigen::Vector3d temp;
      temp << it.x, it.y, 0;
    ...
    for (int i = 0; i < size; i++) {
      double x = bresenhams_line[i](0);
      double y = bresenhams_line[i](1);

      for (double i = x - 0.2; i < x + 0.2; i += 0.05) {
        for (double j = y - 0.2; j < y + 0.2; j += 0.05) {
          int index = output_map.get_index_from_xy(i, j);
          if (index > output_map.get_size() || index < 0) {
            continue;
          } else {
            output_map.data[index] = 100;
          }
        }
      }
    }
  ...
  bool debug = true;
  if (debug) {
    using_occupancy_map_.data.clear();
    using_occupancy_map_.info = input_map.info;
    using_occupancy_map_.header = input_map.header;
    for (const auto &data: output_map.data) {
      using_occupancy_map_.data.emplace_back(data);
    }
    occupancy_grid_pub.publish(using_occupancy_map_);
  }
}
